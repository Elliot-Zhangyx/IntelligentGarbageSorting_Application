    ...
    public List<Garbage> getInfoByName2(String name) {
        ...
        try {
            ...
            if(name.trim().length()==2){
                while(flag<=name.trim().length()) {
                    String name_3=name.substring(j,k);
                    sql="select * from garbages where name like \"%" + name_3 + "%\"";
                    if (connection != null) {// connection不为null表示与数据库建立了连接
                        PreparedStatement ps = connection.prepareStatement(sql);
                        if (ps != null) {
                            // 执行sql查询语句并返回结果集
                    ...
                    while(i<=name.trim().length()){
                        String name_2=name.substring(name.trim().length()-i);
                       // sql = "select * from garbages where name like \"%" + name_2 + "%\"";
                        sql = "select * from garbages where name like \"%" + name_2 + "%\"";
                        System.out.println(sql);
                        if (connection != null) {// connection不为null表示与数据库建立了连接
                            PreparedStatement ps = connection.prepareStatement(sql);
